...
# 지각 발송 마감 (현지 시각).
#
# 2026-08-07 국장 브리핑이 통째로 안 나갔다. 슬롯 7개를 04:20~08:20 KST 에 240분
# 간격으로 깔아 뒀는데, 그날 GitHub 은 슬롯을 하나씩 밀어서 깨우지 않고 **한
# 덩어리로 모아** 09:44~10:45 KST 에 방출했다. 240분짜리 배치가 61분으로 접히면서
# 7개가 전부 창 뒤에 떨어졌고, gate 는 설계대로 전부 버렸다.
#
# 슬롯을 더 까는 것으로는 이 형태를 못 막는다 — 몰려서 늦으면 같이 늦는다. 그래서
# 창을 놓친 날은 "늦었다"고 표시해서라도 그날 한 번은 보낸다 (pipeline/gate.py
# `send_mode`). 마감을 개장 3시간 뒤로 잡은 이유: 그 뒤로는 "장전 브리핑"이라는
# 전제가 너무 무너져서, 보내는 쪽이 안 보내는 쪽보다 더 헷갈린다.
BRIEF_LATE_CUTOFF = {"KR": "12:00", "US": "12:30"}
...
US_UNIVERSE: list[tuple[str, str, str, list[str]]] = [
    ...
    # --- 2026-08-04 확장 (심볼은 전부 FinanceDataReader 로 조회 확인) ---
    ("BRK-B", "Berkshire Hathaway", "NYSE", ["버크셔", "버크셔해서웨이"]),
    ("JNJ", "Johnson & Johnson", "NYSE", ["존슨앤드존슨"]),
    ("PG", "Procter & Gamble", "NYSE", ["P&G", "프록터앤갬블"]),
    ("HD", "Home Depot", "NYSE", ["홈디포"]),
    ("ABT", "Abbott", "NYSE", ["애보트"]),
    ("ACN", "Accenture", "NYSE", ["액센츄어"]),
    ("LIN", "Linde", "NASDAQ", ["린데"]),
    ("TMO", "Thermo Fisher", "NYSE", ["써모피셔"]),
    ("DHR", "Danaher", "NYSE", ["다나허"]),
    ("VZ", "Verizon", "NYSE", ["버라이즌"]),
    ("T", "AT&T", "NYSE", ["AT&T"]),
    ("CMCSA", "Comcast", "NASDAQ", ["컴캐스트"]),
    ...
    ("GS", "Goldman Sachs", "NYSE", ["골드만삭스"]),
    ("MS", "Morgan Stanley", "NYSE", ["모건스탠리"]),
    ("BLK", "BlackRock", "NYSE", ["블랙록"]),
    ("AXP", "American Express", "NYSE", ["아멕스", "아메리칸익스프레스"]),
    ("C", "Citigroup", "NYSE", ["씨티그룹"]),
    ("WFC", "Wells Fargo", "NYSE", ["웰스파고"]),
    ("SCHW", "Charles Schwab", "NYSE", ["찰스슈왑"]),
    ("CAT", "Caterpillar", "NYSE", ["캐터필러"]),
    ("DE", "Deere", "NYSE", ["디어", "존디어"]),
    ("HON", "Honeywell", "NASDAQ", ["하니웰"]),
    ("GE", "GE Aerospace", "NYSE", ["GE", "제너럴일렉트릭"]),
    ("RTX", "RTX", "NYSE", ["레이시온"]),
    ("LMT", "Lockheed Martin", "NYSE", ["록히드마틴"]),
    ("UPS", "UPS", "NYSE", ["UPS"]),
    ("LOW", "Lowe's", "NYSE", ["로우스"]),
    ("SBUX", "Starbucks", "NASDAQ", ["스타벅스"]),
    ("CVS", "CVS Health", "NYSE", ["CVS"]),
    ("BMY", "Bristol-Myers Squibb", "NYSE", ["BMS", "브리스톨마이어스"]),
    ("AMAT", "Applied Materials", "NASDAQ", ["어플라이드머티어리얼즈"]),
    ("LRCX", "Lam Research", "NASDAQ", ["램리서치"]),
    ("KLAC", "KLA", "NASDAQ", ["KLA"]),
    ("ADI", "Analog Devices", "NASDAQ", ["아나로그디바이스"]),
    ("PANW", "Palo Alto Networks", "NASDAQ", ["팔로알토"]),
    ("ANET", "Arista Networks", "NYSE", ["아리스타"]),
    ("MRVL", "Marvell", "NASDAQ", ["마벨"]),
    ("ABNB", "Airbnb", "NASDAQ", ["에어비앤비"]),
    ("PYPL", "PayPal", "NASDAQ", ["페이팔"]),
    ("HOOD", "Robinhood", "NASDAQ", ["로빈후드"]),
    ("F", "Ford", "NYSE", ["포드"]),
    ("GM", "General Motors", "NYSE", ["GM", "제너럴모터스"]),
    ("NEE", "NextEra Energy", "NYSE", ["넥스트에라"]),
    ("PLD", "Prologis", "NYSE", ["프로로지스"]),
    ("AMT", "American Tower", "NYSE", ["아메리칸타워"]),
    ("SPOT", "Spotify", "NYSE", ["스포티파이"]),
    ("CEG", "Constellation Energy", "NASDAQ", ["컨스텔레이션에너지"]),
    ("VST", "Vistra", "NYSE", ["비스트라"]),
    ("APP", "AppLovin", "NASDAQ", ["앱러빈"]),
]
...
US_ETF: list[tuple[str, str, str, list[str]]] = [
    ...
    # --- 2026-08-04 확장: 섹터·채권·해외·테마·레버리지 ---
    ("XLK", "Technology Select SPDR", "ETF", ["기술주 ETF"]),
    ("XLF", "Financial Select SPDR", "ETF", ["금융 ETF"]),
    ("XLE", "Energy Select SPDR", "ETF", ["에너지 ETF"]),
    ("XLV", "Health Care Select SPDR", "ETF", ["헬스케어 ETF"]),
    ("XLI", "Industrial Select SPDR", "ETF", ["산업재 ETF"]),
    ("XLY", "Consumer Discretionary SPDR", "ETF", ["경기소비재 ETF"]),
    ("XLP", "Consumer Staples SPDR", "ETF", ["필수소비재 ETF"]),
    ("XLU", "Utilities Select SPDR", "ETF", ["유틸리티 ETF"]),
    ("XLB", "Materials Select SPDR", "ETF", ["소재 ETF"]),
    ("XLRE", "Real Estate Select SPDR", "ETF", ["리츠 ETF"]),
    ("XLC", "Communication Services SPDR", "ETF", ["통신 ETF"]),
    ("TLT", "iShares 20+ Year Treasury", "ETF", ["미국 장기국채 ETF"]),
    ("IEF", "iShares 7-10 Year Treasury", "ETF", ["미국 중기국채 ETF"]),
    ("SHY", "iShares 1-3 Year Treasury", "ETF", ["미국 단기국채 ETF"]),
    ("AGG", "iShares Core US Aggregate Bond", "ETF", ["미국 종합채권 ETF"]),
    ("LQD", "iShares Investment Grade Corp", "ETF", ["투자등급 회사채 ETF"]),
    ("HYG", "iShares High Yield Corp", "ETF", ["하이일드 ETF"]),
    ("TIP", "iShares TIPS Bond", "ETF", ["물가연동채 ETF"]),
    ("EFA", "iShares MSCI EAFE", "ETF", ["선진국 ETF"]),
    ("EEM", "iShares MSCI Emerging Markets", "ETF", ["신흥국 ETF"]),
    ("VEA", "Vanguard Developed Markets", "ETF", ["선진국 ETF"]),
    ("VWO", "Vanguard Emerging Markets", "ETF", ["신흥국 ETF"]),
    ("EWJ", "iShares MSCI Japan", "ETF", ["일본 ETF"]),
    ("EWY", "iShares MSCI South Korea", "ETF", ["한국 ETF"]),
    ("INDA", "iShares MSCI India", "ETF", ["인도 ETF"]),
    ("FXI", "iShares China Large-Cap", "ETF", ["중국 ETF"]),
    ("MCHI", "iShares MSCI China", "ETF", ["중국 ETF"]),
    ("XBI", "SPDR S&P Biotech", "ETF", ["바이오텍 ETF"]),
    ("IBB", "iShares Biotechnology", "ETF", ["바이오 ETF"]),
    ("ITA", "iShares US Aerospace & Defense", "ETF", ["방산 ETF"]),
    ("ICLN", "iShares Global Clean Energy", "ETF", ["친환경에너지 ETF"]),
    ("TAN", "Invesco Solar", "ETF", ["태양광 ETF"]),
    ("LIT", "Global X Lithium & Battery", "ETF", ["2차전지 ETF", "리튬 ETF"]),
    ("JETS", "US Global Jets", "ETF", ["항공 ETF"]),
    ("BOTZ", "Global X Robotics & AI", "ETF", ["로봇 ETF"]),
    ("IGV", "iShares Expanded Tech-Software", "ETF", ["소프트웨어 ETF"]),
    ("SQQQ", "ProShares UltraPro Short QQQ", "ETF", ["나스닥100 3배 인버스"]),
    ("SOXL", "Direxion Semiconductor Bull 3X", "ETF", ["반도체 3배"]),
    ("SOXS", "Direxion Semiconductor Bear 3X", "ETF", ["반도체 3배 인버스"]),
    ("UPRO", "ProShares UltraPro S&P500", "ETF", ["S&P500 3배"]),
    ("TMF", "Direxion 20+ Treasury Bull 3X", "ETF", ["미국채 3배"]),
    ("SLV", "iShares Silver Trust", "ETF", ["은 ETF"]),
    ("USO", "United States Oil Fund", "ETF", ["원유 ETF"]),
    ("VNQ", "Vanguard Real Estate", "ETF", ["미국 리츠 ETF"]),
    ("VYM", "Vanguard High Dividend Yield", "ETF", ["고배당 ETF"]),
    ("JEPI", "JPMorgan Equity Premium Income", "ETF", ["제피", "커버드콜 ETF"]),
    ("QYLD", "Global X NASDAQ 100 Covered Call", "ETF", ["큐일드", "커버드콜"]),
    ("VUG", "Vanguard Growth", "ETF", ["성장주 ETF"]),
    ("VTV", "Vanguard Value", "ETF", ["가치주 ETF"]),
    ("IJH", "iShares Core S&P Mid-Cap", "ETF", ["미국 중형주 ETF"]),
    ("IJR", "iShares Core S&P Small-Cap", "ETF", ["미국 소형주 ETF"]),
    ("RSP", "Invesco S&P 500 Equal Weight", "ETF", ["동일가중 S&P500"]),
]
